...
axs[1].plot(df1)
axs[1].set_title('Стоимость покупок с 2018 по 2020 годы')
plt.xlabel('Дата покупки')
...
for i in range(800):
    modifier = random.randint(-100, 105) / 800 + 1
    last_close *= modifier
...
axs[2].plot(df1['Price_sum'])
axs[2].plot(df1['Forecast'])
axs[2].set_title('Предсказанная стоимость покупок')
plt.xlabel('Дата покупки')
